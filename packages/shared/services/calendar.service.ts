...
import { CALENDAR_ENDPOINTS, calendarEventById } from '../endpoints/calendar.endpoints';
import type {
  CalendarEvent,
  EventQuery,
  PickCreateEvent,
...
class CalendarService {
  public async ListEvents(query?: EventQuery): Promise<TResponse<CalendarEvent[]>> {
    const res = await GetResponse<CalendarEvent[]>(withQuery(CALENDAR_ENDPOINTS.LIST, query));
    return toServiceResponse(res, {
      message: 'Daftar jadwal berhasil diambil',
    });
  }
  public async CreateEvent(payload: PickCreateEvent): Promise<TResponse<CalendarEvent>> {
    const res = await PostResponse<CalendarEvent>(CALENDAR_ENDPOINTS.CREATE, payload);
    return toServiceResponse(res, {
  ...
  public async UpdateEvent(
    ...
    const res = await PatchResponse<CalendarEvent>(calendarEventById(id.id), payload);
