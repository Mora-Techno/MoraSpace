...
import type { ITeam, PickCreateTeam, PickUpdateTeam, PickAddTeamMember } from "../types/team.types";
import type { TResponse } from "../types/response.types";
import { DeleteResponse, GetResponse, PatchResponse, PostResponse, withQuery } from "./http";
import { toServiceResponse } from "./service-response";

class TeamService {
  public async ListTeams(query?: { departmentId?: string }): Promise<TResponse<ITeam[]>> {
    const res = await GetResponse<ITeam[]>(withQuery(TEAM_ENDPOINTS.LIST, query));
  ...
  public async CreateTeam(payload: PickCreateTeam): Promise<TResponse<ITeam>> {
    ...
    return toServiceResponse(res, { message: "Tim berhasil dibuat", statusCode: 201 });
  }

  public async UpdateTeam(id: string, payload: PickUpdateTeam): Promise<TResponse<ITeam>> {
