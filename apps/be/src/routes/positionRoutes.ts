...
import { CreatePositionDto, PositionParamsDto, UpdatePositionDto } from '@/dto/position.dto';
...
class PositionRouter {
  ...
  private routes() {
    this.positionRouter.get('/', (c: AppContext) => PositionController.list(c), {
    ...
    this.positionRouter.post('/', (c: AppContext) => PositionController.create(c), {
    ...
    this.positionRouter.patch('/:id', (c: AppContext) => PositionController.update(c), {
    ...
    this.positionRouter.delete('/:id', (c: AppContext) => PositionController.remove(c), {
      params: PositionParamsDto,
      beforeHandle: [verifyToken().beforeHandle],
      detail: {
