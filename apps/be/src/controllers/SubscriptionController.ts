...
class SubscriptionController {
  public async listPlans(c: AppContext) {
    try {
      ...
      return HttpResponse(c).ok(queryService, undefined, 'Berhasil mengambil daftar paket');
    } catch (error) {
      console.error(error);
  ...
  public async getMine(c: AppContext) {
    try {
      ...
      return HttpResponse(c).ok(queryService, undefined, 'Berhasil mengambil langganan');
    } catch (error) {
      console.error(error);
      return HttpResponse(c).internalError(error);
    }
  ...
  public async checkout(c: AppContext) {
    try {
      ...
      if (user.companyRole !== 'Owner') {
        return HttpResponse(c).forbidden('Hanya owner yang dapat mengelola langganan');
      }
      ...
      return HttpResponse(c).ok(queryService, undefined, 'Checkout berhasil dibuat');
    } catch (error) {
      return HttpResponse(c).internalError(error);
    }
  }
  ...
  public async cancel(c: AppContext) {
    try {
      ...
      if (user.companyRole !== 'Owner') {
        return HttpResponse(c).forbidden('Hanya owner yang dapat membatalkan langganan');
      }
      ...
      const queryService = await SubscriptionService.cancelSubscription(user.companyId);
      return HttpResponse(c).ok(queryService, undefined, 'Langganan berhasil dibatalkan');
    } catch (error) {
      console.error(error);
  ...
  public async stripeWebhook(c: AppContext) {
    try {
      ...
      const queryService = await SubscriptionService.handleStripeWebhook(payload, signature);

      return HttpResponse(c).ok(queryService, undefined, 'Webhook Stripe diterima');
    } catch (error) {
      console.error(error);
  ...
  public async xenditWebhook(c: AppContext) {
    try {
      ...
      return HttpResponse(c).ok(queryService, undefined, 'Webhook Xendit diterima');
    } catch (error) {
      console.error(error);
